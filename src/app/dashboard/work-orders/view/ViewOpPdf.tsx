...
import { getOperationPartPdf } from "@/lib";
import { ScrollArea } from "@/ui";
import { handleForm } from "@/lib";
import PdfModal from "./PdfModal";

function ViewOperationPartPdf({ operationPartId, customer }: any) {
  ...
  function handleClickPdf(opId: string) {
    setPdfOpen(true);
    (async () => {
      const session = await getSession();
      if (!session) {
      ...
      const result = await getOperationPartPdf(token, opId);
  ...
  return (
    <>
      <div className="flex">
        <Button
          onClick={() => handleClickPdf(operationPartId)}
          variant="outline"
...
export default ViewOperationPartPdf;
